...
<div class="container-fluid section">
    <h3 class="text-center main-title">
        <i class="glyphicon glyphicon-user"></i>&nbsp;&nbsp;VIEW RENT PREPAID
    </h3>
        ...
        <div class="modal-dialog large-modal">
            <div class="modal-content" style="width: 1100px;margin-left: -200px">
                <div class="modal-header">
                    <button type="button" class="btn btn-warning" onclick="screenshotrentprepaid('page-content');">Download
                    </button>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title" id="myModalLabelViewContributions" style="text-align: center">
                        <i class="glyphicon glyphicon-picture"></i>&nbsp;&nbsp; VIEW RENT PREPAID
                    </h4>
                </div>
...
<script type="text/javascript">
    ...
    function screenshotrentprepaid(data) {
        html2canvas(document.getElementById(data)).then(function (canvas) {
            ...
            pdf.save("Rent Paid Prepaid.pdf");
    ...
    $(document).ready(function () {
        ...
        $('#sa-form').bootstrapValidator({
            ...
            $.ajax({
                ...
                type: 'post',
                data: {
                    ACTION: 'VIEW_RENT_PREPAID_GRID_ADMIN',
                    fromDate: $('#fromDate')
                        .val(),
                    toDate: $('#toDate')
                        .val()
                ...
                success: function (json) {
                    console.log(json);
                    html = "<tr><th>TENANT NAME</th><th>UNIT NUMBER</th><th>PREPAID</th><th>MONTH</th><th>YEAR</th></tr>";
                    var prepaidAmountSum = 0;
                    var totalSum = 0;
                    if (json.success) {
                        ...
                        $.each(json, function (key, value) {
                            if (key == 'rows') {
                                for (var i = 0; i < json.rows.length; i++) {
                                    var row = json.rows[i];
                                    var prepaidAmount = (row['prepaidAmount'] != null ? row['prepaidAmount'] : 0) || (isEmpty(row['prepaidAmount']) ? 0 : row['prepaidAmount']);
                                    html = html + "<tr><td>" + row['tenantName'] + "</td><td>" + row['unitNumber'] + "</td><td>" + format_no(prepaidAmount) +
                                        "</td><td>" + row['month'] + "</td><td>" + row['year'] + "</td><td></tr>";

                                    prepaidAmountSum = prepaidAmountSum + parseFloat(prepaidAmount);


                                }

                                empty = "<tr><th></th><th></th><th>TOTAL PREPAID AMOUNT</th><th></th><th></th></tr>";
                                totals = "<tr><td></td><td></td><td>" + format_no(precisionRound(prepaidAmountSum, 1)) + "</td><td></td><td></td></tr>";
                ...
                error:function () {
                    bootbox.alert('<p class="text-center">' + "No rent arrears paid for the selected period!" + '</p>');
                }
            });
        });
