					...
					<div class="col-md-4 hide" id="operations">
						<select class="form-control" id="operation" onchange="do_function();">
							...
							<c:if test="${ permissions.operation_unitized_statement }">
								<option>UNITIZED MEMBER STATEMENT</option>
							</c:if>
 							<c:if test="${ permissions.operation_individual_member_statement }">
								<option>INDIVIDUAL MEMBER STATEMENT</option>
							</c:if>
...
function do_function()
{
    ...
    else if($('#operation').val() == 'UNITIZED MEMBER STATEMENT')
    {
        start_wait();
        $.ajax({
            url: $('#base_url').val() + 'dashboard',
    ...
    else if($('#operation').val() == 'INDIVIDUAL MEMBER STATEMENT')
    {
        start_wait();
        $.ajax({
            url: $('#base_url').val() + 'dashboard',
            type: 'get',
            data: {dashboard: 'IMS', memberID: member_id},
            dataType: 'html',
            success: function(html) {
                $('#page-info').html(html);
                stop_wait();
...
function select_member(id, name, member_no)
{
	...
	$('#modal-view-agent').modal('hide');
	$('#operations').removeClass('hide');
}
$(document).ready(function(){
$('#agent-search').bootstrapValidator({
