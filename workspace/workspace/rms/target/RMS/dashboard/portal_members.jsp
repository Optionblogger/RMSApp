...
{
	bootbox.confirm("<p class='text-center'>You have requested to delete the potential agent. Are you sure?</p>", function(result){
		if(result)
		{
		start_wait();
		$.ajax({
...
{
	bootbox.confirm("<p class='text-center'>You have requested to submit agent to FundMaster Xi. Are you sure?</p>", function(result){
		if(result)
		{
		start_wait();
		$.ajax({
...
$(document).ready(function(){
	...
	.on('changeDate', function(e) {
	    ...
	    $('#form-new-agent').bootstrapValidator('revalidateField', 'dateOfBirth');
	});
	$('#form-new-agent')
			.bootstrapValidator(
					{
			...
			.on(
					...
					function(e) {
						...
						e.preventDefault();
						 ...
						 var form = "form-new-agent";
			             var modal = "modal-new-agent";
